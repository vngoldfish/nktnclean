...
def create_circular_avatar(logo_path, size=36):
    ...
    # We want the logo to occupy about 62% of the circle (around 22px)
...
def patch_report():
    ...
    # 1. Paint over "OSJ" text under top avatar: y = 311 to 320, x = 345 to 380
    bg_color_top = img.getpixel((340, 315))
    draw.rectangle([344, 311, 381, 321], fill=bg_color_top)
    
    # 2. Paint over "OSJ" text under bottom avatar: y = 751 to 760, x = 345 to 380
    bg_color_bottom = img.getpixel((340, 755))
    draw.rectangle([344, 751, 381, 761], fill=bg_color_bottom)
    
    # 3. Paint over [OS] text at first message: y=424..435, x=480..520
    bg_color_1 = img.getpixel((460, 430))
    draw.rectangle([475, 422, 520, 436], fill=bg_color_1)
    
    # 4. Paint over [OSJ] text at second message: y=554..565, x=478..520
    bg_color_2 = img.getpixel((460, 560))
    draw.rectangle([478, 552, 520, 566], fill=bg_color_2)
    
    # 5. Paint over [OSJ] text at third message: y=674..685, x=485..525
    bg_color_3 = img.getpixel((460, 680))
    draw.rectangle([485, 672, 525, 686], fill=bg_color_3)
