    ...
    <main className="site-shell overflow-hidden">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-3">
          {content.services.map((service, index) => <Card key={service.title} className="p-7"><p className="text-sm font-black tracking-[0.2em] text-nktn-blue">0{index + 1}</p><CardTitle className="mt-10 text-3xl tracking-[-0.05em]">{service.title}</CardTitle><p className="mt-5 leading-8 text-nktn-ink/64">{service.lead}</p></Card>)}
        </div>
      </section>
      ...
      <section className="mx-5 rounded-3xl border border-slate-200 bg-white px-5 py-20 shadow-soft sm:mx-8 sm:px-8"><div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1fr_1fr] lg:items-center"><div><Badge variant="orange" className="mb-6">Why NKTN</Badge><h2 className="whitespace-pre-line text-balance text-4xl font-black tracking-[-0.04em] text-nktn-ink sm:text-6xl">{content.home.strengthsTitle}</h2><Button className="mt-8" asChild><Link href={withLocale(locale, "/strengths")}>{content.nav[2][0]} <ArrowRight className="size-4" /></Link></Button></div><div className="grid gap-4 sm:grid-cols-2">{content.strengths.map((strength) => <div key={strength.title} className="rounded-2xl bg-slate-50 p-5 ring-1 ring-slate-200"><p className="font-black text-nktn-ink">{strength.title}</p><p className="mt-3 text-sm leading-6 text-nktn-ink/64">{strength.body}</p></div>)}</div></div></section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20 sm:px-8"><div className="grid gap-4 lg:grid-cols-4">{content.processSteps.map(([number, title, body]) => <Card key={number} className="p-6"><p className="text-sm font-black tracking-[0.2em] text-nktn-blue">{number}</p><h3 className="mt-10 text-xl font-black">{title}</h3><p className="mt-4 leading-7 text-nktn-ink/62">{body}</p></Card>)}</div></section>
    </main>
  );
}
