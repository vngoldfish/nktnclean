...
export default async function StrengthsPage({ params }: { params: Promise<{ locale: Locale }> }) {
  ...
  return (
    <main className="site-shell">
      <section className="mx-5 my-10 rounded-[2.5rem] bg-nktn-ink px-5 py-20 text-white sm:mx-8 sm:px-8 lg:rounded-[4rem] lg:py-28">
        <div className="mx-auto max-w-7xl">
          <Badge variant="orange" className="mb-6 bg-nktn-orange text-nktn-ink ring-0">{content.strengthsPage.badge}</Badge>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-24 sm:px-8">
          ...
          {content.strengths.map((strength, index) => {
            ...
            return (
              <Card key={strength.title} className="p-7">
                <Icon className="size-8 text-nktn-orange" />
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-3">
          {[
            ["100%", "Report / contact / completion management"],
            ["ON TIME", "Deadline-oriented cleaning design"],
            ["BIG OPS", "DX-powered field operation"],
          ].map(([label, body]) => (
            <div key={label} className="rounded-[2rem] bg-white p-7 shadow-soft">
              <p className="text-4xl font-black tracking-[-0.04em] text-nktn-blue">{label}</p>
        ...
        <Button className="mt-10" asChild>
          <Link href={companyBase.lineUrl}>
            {content.strengthsPage.cta} <ArrowRight className="size-4" />
          </Link>
        </Button>
      </section>
    </main>
