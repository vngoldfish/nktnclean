        ...
        <section className="mx-auto max-w-7xl px-5 pb-16 sm:px-8">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            ...
            <Card className="overflow-hidden p-0">
              <Image src={post.image} alt={post.title} width={900} height={620} className="h-[28rem] w-full object-cover" priority />
            </Card>
          </div>
        </section>
        ...
        <section className="mx-auto grid max-w-7xl gap-8 px-5 pb-24 sm:px-8 lg:grid-cols-[1fr_22rem]">
          <div className="rounded-[2.5rem] bg-white p-7 shadow-soft lg:p-10">
            <div className="grid gap-4">
              {post.points.map((point) => <div key={point} className="flex gap-3 rounded-2xl bg-nktn-cream p-4 font-black leading-7 text-nktn-ink/74"><CheckCircle2 className="mt-1 size-5 shrink-0 text-nktn-green" />{point}</div>)}
            </div>
            ...
            <div className="mt-12 rounded-[2rem] bg-nktn-ink p-7 text-white">
              <div className="flex gap-4">
                <ShieldCheck className="mt-1 size-7 shrink-0 text-nktn-orange" />
                <div>
          ...
          <aside className="space-y-5">
            <Card className="p-6">
              <p className="text-sm font-black tracking-[0.18em] text-nktn-blue">CONTACT</p>
              ...
              <Button className="mt-6 w-full bg-nktn-green hover:bg-[#438b62]" asChild><Link href={companyBase.lineUrl}>{content.common.lineConsult} <ArrowRight className="size-4" /></Link></Button>
            ...
            <Card className="p-6">
              <p className="text-sm font-black tracking-[0.18em] text-nktn-orange">RELATED</p>
              <div className="mt-5 space-y-4">
                {relatedPosts.map((item) => <Link key={item.slug} href={withLocale(locale, `/blog/${item.slug}`)} className="block rounded-2xl bg-nktn-cream p-4 font-black leading-7 transition hover:bg-nktn-orange/15">{item.title}</Link>)}
              </div>
            </Card>
          </aside>
        </section>
